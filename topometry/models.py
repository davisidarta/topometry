...
from topometry.base import ann as nn
...
class TopoGraph(TransformerMixin, BaseEstimator):
    ...
    def __repr__(self):
        ...
        if self.DiffBasis is None:
            return print("TopoGraph object without any fitted model" % (self.a, self.b))
        if self.DiffBasis is not None:
            base_print = base_print + " \n DiffusionBasis fitted on %f samples and %f observations" % (self.DiffBasis.shape[0] / self.DiffBasis.shape[0])
    ...
    classes for graph layout optimization in `topometry.layout`.
