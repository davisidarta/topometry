...
Let's load the PBMC68k dataset. It can be downloaded from the [10X Genomics website](https://www.10xgenomics.com/datasets/fresh-68-k-pbm-cs-donor-a-1-standard-1-1-0).

Once the data is saved to our working directory, we can unzip and read it:
...
```python
adata = sc.read_10x_mtx('filtered_matrices_mex/hg19/')
adata
```
...
![png](T2_step_by_step_files/T2_step_by_step_12_0.png)
...
![png](T2_step_by_step_files/T2_step_by_step_16_0.png)
...
```
    ...
    /home/davi/intelpython3/lib/python3.9/site-packages/topo/single_cell.py:560: ImplicitModificationWarning: Setting element`.layers['counts']` of view, initializing view as actual.
      adata.layers["counts"] = adata.X.copy()

    AnnData object with n_obs × n_vars = 66653 × 3000
...
sc.pl.dotplot(adata, marker_dict, groupby="predicted_celltype", standard_scale="var", dendrogram=True)
```
...
```
    ...
    2026-03-03 15:20:59.173393: I tensorflow/core/util/util.cc:169] oneDNN custom operations are on. You may see slightly different numerical results due to floating-point round-off errors from different computation orders. To turn them off, set the environment variable`TF_ENABLE_ONEDNN_OPTS=0`.
...
```
    ...
    /home/davi/intelpython3/lib/python3.9/site-packages/topo/single_cell.py:789: FutureWarning: In the future, the default backend for leiden will be igraph instead of leidenalg.

    To achieve the future defaults please pass: flavor="igraph" and n_iterations=2.  directed must also be False to work with igraph's implementation.
      sc.tl.leiden(adata, resolution=res, adjacency=P, key_added=key)

    AnnData object with n_obs × n_vars = 66653 × 3000
        obs: 'n_genes_by_counts', 'log1p_n_genes_by_counts', 'total_counts', 'log1p_total_counts', 'pct_counts_in_top_50_genes', 'pct_counts_in_top_100_genes', 'pct_counts_in_top_200_genes', 'pct_counts_in_top_500_genes', 'total_counts_mt', 'log1p_total_counts_mt', 'pct_counts_mt', 'total_counts_ribo', 'log1p_total_counts_ribo', 'pct_counts_ribo', 'total_counts_hb', 'log1p_total_counts_hb', 'pct_counts_hb', 'pca_leiden', 'topo_clusters_res0.2', 'topo_clusters_res0.8', 'topo_clusters', 'topo_clusters_ms_res0.2', 'topo_clusters_ms_res0.8', 'topo_clusters_ms'
        var: 'gene_ids', 'mt', 'ribo', 'hb', 'n_cells_by_counts', 'mean_counts', 'log1p_mean_counts', 'pct_dropout_by_counts', 'total_counts', 'log1p_total_counts', 'highly_variable', 'highly_variable_rank', 'means', 'variances', 'variances_norm', 'mean', 'std'
        uns: 'log1p', 'hvg', 'pca', 'pca_leiden', 'umap', 'pca_leiden_colors', '_topo_tmp_dm', 'topo_clusters_res0.2', 'topo_clusters_res0.8', '_topo_tmp_ms', 'topo_clusters_ms_res0.2', 'topo_clusters_ms_res0.8'
...
![png](T2_step_by_step_files/T2_step_by_step_41_0.png)

TopoMetry finds a surprisingly high number of T cell populations in this dataset. Interestingly, some of these cells were classified as "Tfh" (T folicular helper).
...
```
    ...
    <matplotlib.patches.Circle at 0x75d77976ecd0>
...
```python
sc.pl.embedding(adata, basis='TopoMAP', color=["CXCR5", "PDCD1", "ICOS"], cmap='inferno', vmin=0, frameon=False)
```

![png](T2_step_by_step_files/T2_step_by_step_45_0.png)

As we can see, that population presents highly specific expression of PDCD1, which has biased our simple classifier into calling them "Tfh". However, they don't present specific expression of other Tfh markers (CXCR5, ICOS).
...
```
    ...
    'msTopoMAP_training_1771252989.gif'
...
```
    ...
    Top 10 genes for SC_0 (by |corr|):
                   SC_0      corr
    CD79A      0.892053 -0.892053
    TCL1A      0.755219 -0.755219
    MS4A1      0.749220 -0.749220
    CD79B      0.749116 -0.749116
    HLA-DRA    0.712188 -0.712188
    LINC00926  0.613364 -0.613364
    HLA-DQA1   0.607347 -0.607347
    HLA-DPA1   0.581358 -0.581358
    HLA-DRB1   0.573755 -0.573755
    VPREB3     0.573118 -0.573118
...
```python
sc.pl.embedding(adata, basis='TopoMAP', color=['IL7R','CD8A','NKG7','FCER1A'], cmap='inferno', layer='scaled', vmin=0)
```

![png](T2_step_by_step_files/T2_step_by_step_71_0.png)

```python
sc.pl.embedding(adata, basis='TopoMAP', color=['IL7R','CD8A','NKG7','FCER1A'], cmap='inferno', layer='topo_imputation', vmin=0)
```
...
Because the example dataset (pbmc68k) consists of cells from a single healthy donor, it does not contain a naturally varying disease or condition label that could be used to illustrate graph signal filtering. To demonstrate the method in a controlled setting, we therefore simulate a categorical **disease-state** signal directly on the cell graph. Specifically, a small number of clusters are selected at random, and within these “hotspot” clusters a large fraction of cells (e.g., ~70%) are labeled as *diseased*, while the remaining clusters receive a much lower background rate of disease labeling (e.g., 10%). This construction creates a structured but imperfect signal that is enriched in specific regions of the manifold, mimicking how disease-associated states or perturbations often appear in real single-cell datasets.
...
</style><div id="sk-container-id-2" class="sk-top-container"><div class="sk-text-repr-fallback"><pre>TopOGraph object with 66653 samples and 3000 observations and:
